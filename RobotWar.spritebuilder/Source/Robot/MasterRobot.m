//
//  MasterRobot.m
//  RobotWar
//
//  Created by Johnny Quinn on 7/3/14.
//  Copyright (c) 2014 Apportable. All rights reserved.
//

#import "MasterRobot.h"

typedef NS_ENUM(NSInteger, RobotState) {
    RobotStateDefault,
    RobotStateTurnaround,
    RobotStateFiring,
    RobotStateSearching
};

@implementation MasterRobot {
    RobotState _currentRobotState;
    
    CGPoint _lastKnownPosition;
    CGFloat _lastKnownPositionTimestamp;
    CGPoint _currentPosition;
    CGFloat _currentAngle;
}

- (void)run {
    while (true) {
        if (_currentRobotState == RobotStateFiring) {
            
            if ((self.currentTimestamp - _lastKnownPositionTimestamp) > 1.f) {
                _currentRobotState = RobotStateSearching;
            } else {
                CGFloat angle = [self angleBetweenGunHeadingDirectionAndWorldPosition:_lastKnownPosition];
                if (angle >= 0) {
                    [self turnGunRight:abs(angle)];
                } else {
                    [self turnGunLeft:abs(angle)];
                }
                [self shoot];
                [self moveAhead:40];
            }
        }
        //Moves to _lastKnownLocaiton
        if (_currentRobotState == RobotStateSearching) {
            _currentPosition = [self position];
            CGFloat angle = [self angleBetweenHeadingDirectionAndWorldPosition:_lastKnownPosition];
            NSLog(@"Working");
            if(_currentPosition.x > _lastKnownPosition.x) {
                if (angle > 0) {
                    [self turnRobotRight:abs(angle)];
                }
                else {
                    [self turnRobotLeft:abs(angle)];
                }
                int targetPositionX = _currentPosition.x - _lastKnownPosition.x;
                [self moveAhead:abs(targetPositionX)];
            }
            else {
                if(angle > 0) {
                    [self turnRobotRight:abs(angle)];
                }
                else {
                    [self turnRobotLeft:abs(angle)];
                }
                int targetPositionX = _lastKnownPosition.x - _currentPosition.x;
                [self moveAhead:abs(targetPositionX)];
                
            }
            if (_currentPosition.y > _lastKnownPosition.y) {
                if (angle > 0) {
                    [self turnRobotRight:abs(angle)];
                } else {
                    [self turnRobotLeft:abs(angle)];
                }
                int targetPositionY = _currentPosition.y - _lastKnownPosition.y;
                [self moveAhead:abs(targetPositionY)];
            }
            else {
                if (angle > 0) {
                    [self turnRobotRight:abs(angle)];
                }
                else {
                    [self turnRobotLeft:abs(angle)];
                }
                int targetPositionY = _lastKnownPosition.y - _currentPosition.y;
                [self moveAhead:abs(targetPositionY)];
            }
            _currentRobotState = RobotStateFiring;
        }
        
    }
    
    if (_currentRobotState == RobotStateDefault) {
        [self moveAhead:100];
    }
}

- (void)bulletHitEnemy:(Bullet *)bullet {
    // There are a couple of neat things you could do in this handler
}

- (void)scannedRobot:(Robot *)robot atPosition:(CGPoint)position {
    if (_currentRobotState != RobotStateFiring) {
        [self cancelActiveAction];
    }
    
    _lastKnownPosition = position;
    _lastKnownPositionTimestamp = self.currentTimestamp;
    _currentRobotState = RobotStateFiring;
}

- (void)hitWall:(RobotWallHitDirection)hitDirection hitAngle:(CGFloat)angle {
    if (_currentRobotState != RobotStateTurnaround) {
        [self cancelActiveAction];
        
        RobotState previousState = _currentRobotState;
        _currentRobotState = RobotStateTurnaround;
        
        // always turn to head straight away from the wall
        if (angle >= 0) {
            [self turnRobotLeft:abs(angle)];
        } else {
            [self turnRobotRight:abs(angle)];
            
        }
        
        [self moveAhead:20];
        
        _currentRobotState = previousState;
    }
}

@end